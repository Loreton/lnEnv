    ...
    for filepath in Path(topdir).glob(f"**/{file_pattern}"): ### mettere /* in modo che prenda anche i file senza extension e le directories
        ...
        if filepath.is_file():
            ...
            if filepath.is_symlink():
                if args.verbose: print(" - it's a symlink", color.reset)

            elif filepath.suffix in exclude_ext:
                if args.verbose: print(" - extension is ecluded", color.reset)

            elif filepath.name in exclude_pattern:
                if args.verbose: print(" - fname is ecluded", color.reset)
...
# ######################################################
def findText(filepath: str, cur_str: str, new_str: str):
...
def ParseInput():
    ...
    parser = argparse.ArgumentParser(description='replace string in text files')
    ...
    parser.add_argument('--search-string', help='string to be searched', type=str, default=None, required=True, metavar="")
    parser.add_argument('--new-string', help='string to be used on replace', type=str, default=None, required=False, metavar="")
    parser.add_argument('--top-dir', help='/home/path', type=str, default=None, required=True, metavar="")
...
if __name__ == '__main__':
    ...
    if args.new_string is None:
        args.new_string=args.search_string
        # args.go=False

    for filepath in getFileList_glob(topdir=args.top_dir, file_pattern="*"):
        # print(filepath)
        findText(filepath=filepath, cur_str=args.search_string, new_str=args.new_string)
