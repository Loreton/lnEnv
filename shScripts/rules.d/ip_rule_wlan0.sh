...
# When you ping the IP address 192.168.3.176 from outside your network you may notice that it will not be pingable.
...
# Now add the routing entries in the $TABLE table.
    ...
    # 1. In the first ip command, we are adding subnet 192.168.3.0 with a netmask 255.255.255.0 with the source IP address 192.168.3.176 & device wlan1 to the loreto table.
    # 2. In the second ip command, we are adding the route 192.168.3.1 to the $TABLE table. This way all the rules defined in $TABLE table routes traffic through device wlan1.
