...
#############################################################
    ...
    for line in "${g_aLinkList[@]}"; do
        ...
        # ------------------------
        # - se ÃÂÃÂ¨ una dir devo cancellarla altrimenti mi crea
        # - un'infinitÃÂÃÂ  di sub-dirs nel sorgente
        # - ÃÂÃÂ¨ giusto perchÃÂÃÂ© di fatto va a scrivere nel source
        ...
        if [ -d "${real_sourceFile}" ]; then
            ...
            if [ "$curr_ptr" == "${real_sourceFile}" ]; then
                @lnLog "${yellowH}OK --> ${cyanH}${link_name} ${greenH}${real_sourceFile}${colorReset}"
            else
                @lnLog "${cyanH} - it's a directory. ${redH}current logicalLink will be removed!${colorReset}"
                @lnEsegui "rm -rf ${link_name}"
                @lnEsegui "ln -s ${real_sourceFile} ${link_name}"
            fi

        elif [[ ! -f "${real_sourceFile}"  ]]; then
            @lnLog "${redH}${sourceFile} doesn't exists${colorReset}"
            exit 1
            ...
            if [ "$curr_ptr" == "${real_sourceFile}" ]; then
                ...
                @lnLog "${redH}wrong pointer ${cyanH}${link_name} ${colorReset}"
                @lnEsegui "ln -sf ${real_sourceFile} ${link_name}"
            fi


        elif [[ -f "${link_name}"  ]]; then
            @lnLog "${cyanH}${link_name} ${redH}already exists as FILE. Please remove it manually${colorReset}"
            @lnLog "${cyanH}....skipping${colorReset}"

        elif [ -f "${real_sourceFile}" ]; then
            echo -en "${yellowH}"
            @lnEsegui "chmod -w ${real_sourceFile}"
            @lnEsegui "ln -sf ${real_sourceFile} ${link_name}"

        else
            @lnLog "${purpleH}${real_sourceFile} NOT FOUND${colorReset}"
