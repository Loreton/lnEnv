...
function get_PID() {
    ...
    ps_cmd="ps -ef | grep -v 'grep' | grep -i -- /usr/bin/python | grep -i -- '--logging-dir ${LOGGER_DIR}' | grep -i '${TG_GROUP_NAME}' | grep -i $pidFile"
    echo "      $ps_cmd"
    # g_process=$(ps -ef | grep -v 'grep' | grep -i -- "/usr/bin/python" | grep -i -- "--logging-dir ${LOGGER_DIR}" | grep -i ${TG_GROUP_NAME} | grep -i $pidFile )
...
############################################
    ...
    PRJ_NAME="lncasettaBot"
    DIR_NAME="telegramBot"
    ...
    g_prj_dir="${ln_GIT_REPO_DIR}/${DIR_NAME}"
    g_productionProgram="${ln_LIVE_PRODUCTION}/${DIR_NAME}.zip"
    g_tempCompiledProgram="/tmp/compiled/${DIR_NAME}.zip"

    devel_Program="${ln_GIT_REPO_DIR}/${DIR_NAME}/__main__.py"
    compiled_Program="/tmp/compiled/${DIR_NAME}.zip"
