...
class ProjectBuilder:
    def __init__(self, args):
        ...
        self.conf_path       = self.root_dir / "conf"
        self.version         = self.get_version()
        self.dist_dir        = self.root_dir / ".dist"
        self.history_dir     = self.root_dir / ".history" if args.history else None
        self.max_history     = 10

        if self.checkPythonProjectDir():
            print("=" * 40)
            print(f"\t{str(self.root_dir)     = }")
    ...
    def checkPythonProjectDir(self) -> str:
        if not (self.root_dir / ".venv").is_dir():
            print("ERROR: directory .venv not found!")
            return False

        elif not (self.root_dir / "pyproject.toml").is_file():
            print("ERROR: file pyproject.toml not found!")
            return False

        elif not (self.root_dir / "library.json").is_file():
    ...
    def create_pyz(self) -> str:
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            if self.conf_path.exists():
                ...
                shutil.copytree(self.conf_path, temp_dir / "conf",
                              ignore=shutil.ignore_patterns('__pycache__', '*.pyc'))
...
'''
    ...
    def run(self):
        ...
        if self.args.clean:
            ...
            pyz_path = self.create_pyz()
            if self.history_dir:
                self.rotate_previous_build(pyz_path, "pyz")
